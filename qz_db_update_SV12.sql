...
VALUES
('Update to SV12',
'etags to hex, Register Template, XXXXX'
);
...
-- For js and css files, make the etag a hex value 
-- so that any signed 64 bit int will fit in a tag

UPDATE qz.table_action
SET sql = 'SELECT to_hex(etag) "etag"
    FROM qz.css
    WHERE filename = $1'
WHERE form_name = 'css'
AND action = 'etag_value';
...
UPDATE qz.table_action
SET sql =  'SELECT filename, mimetype, modtime, to_hex(etag) "etag", data
    FROM qz.css
    WHERE filename = $1'
WHERE form_name = 'css'
AND action = 'get';
...
WHERE (form_name) IN ('prompt_rule','table_action');
DELETE FROM qz.page_js 
WHERE (form_name) IN ('prompt_rule','table_action');

ALTER TABLE qz.form ALTER COLUMN add_description SET DEFAULT false;

ALTER TABLE qz.page_css
ADD FOREIGN KEY (form_name) REFERENCES qz.form(form_name);

ALTER TABLE qz.page_js
ADD FOREIGN KEY (form_name) REFERENCES qz.form(form_name);
...
GRANT SELECT ON qz.template TO qzuser;
GRANT SELECT ON qz.div_id TO qzuser;

-- fix previous suboptimal practice
