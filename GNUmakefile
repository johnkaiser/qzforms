...
LFLAGS=-lfcgi -lpthread -lcrypto -lxml2 -lgcc
...
XMLLIBDIR:=$(shell xml2-config --libs)

PCRECFLAGS:=$(shell pcre-config --cflags)
...
wtf:
	echo "QZRANDOM=$(QZRANDOM)"
	echo "CC=$(CC)"
	echo "LFLAGS=$(LFLAGS)"
	echo "CFLAGS=$(CFLAGS)"
	echo "PGINCLUDEDIR=$(PGINCLUDEDIR)"
	echo "PGLIBDIR=$(PGLIBDIR)"
	echo "XMLCFLAGS=$(XMLCFLAGS)"
	echo "XMLLIBDIR=$(XMLLIBDIR)"
	echo "PCRECFLAGS=$(PCRECFLAGS)"
	echo "PCRELIBS=$(PCRELIBS)"
...
qzforms.js.sql: $(JS) 
	echo -e "UPDATE qz.js SET data = \\044QZ\\044"  > qzforms.js.sql
	...
	echo -e "\\044QZ\\044 WHERE filename = 'qzforms.js'" >> qzforms.js.sql
