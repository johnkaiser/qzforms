...
struct form_set* get_form_set(struct handler_args* h, struct form_record* form){
    ...
    struct form_set* fs = xmlHashLookup(h->session->form_sets, form->form_set_id);
    return fs;
}
...
// void decrement_form_set(struct handler_args* h, struct form_record* form_rec){
...
void set_context_parameters(struct handler_args* h, struct form_record* new_form_rec,
    PGresult* values_rs, int row) {

    if (h->page_ta == NULL) return;
    // XXXXXZZZ  if (h->page_ta->set_context_parameters == false) return;
    ...
    if (posted_form_rec == NULL) return;

    struct form_set* posted_form_set = get_form_set(h, posted_form_rec);
    struct form_set* new_form_set = NULL;

    if (posted_form_set != NULL){
        
        fprintf(h->log, "%f %d %s:%d posted form set name %s new form set name %s\n",
            gettime(), h->request_id, __func__, __LINE__,
        ...
        if (strcmp(posted_form_set->name, h->page_ta->form_set_name) == 0){
            ...
            fprintf(h->log, "%f %d %s:%d posted and new form set names match. id=%s\n",
                gettime(), h->request_id, __func__, __LINE__,
                posted_form_set->id);
